...
# 100000  818이 베스트인데...
SEED1 = 100000  #999999999  23 111999999 100000
SEED2 = 210         #999999999  818(7.78) 10000987 129 558
# Process 변수와 출력 관련 system gain matrix
...
def main():
    ...
    fwc_p1_vm.DoE_Run(lamda_PLS=v1_PLS, Z=Z_DoE, M=M)  # DoE Run
    Error_VMresult, Error_ACTResult, ez_run, p1_y_act, p1_y_prd, _ = fwc_p1_vm.VM_Run(lamda_PLS=v1_PLS, Z=Z_VM, M=M)
    ...
    p2_VM_Output, p2_ACT_Output, p2_ez_run, p2_y_act, p2_y_prd, reactive_p2_ez_all_run_q2 = fwc_p2_act.VM_Run(lamda_PLS=v2_PLS, Z=Z_VM, M=M)

    np.savetxt("output/reactive_p2_ez_all_run_q2.csv", reactive_p2_ez_all_run_q2, delimiter=",", fmt="%.4f")
    np.savetxt("output/after_normal_p2_ez_all_run_q2.csv", normal_p2_ez_all_run_q2, delimiter=",", fmt="%.4f")

    fdh_graph.plt_show1(Z_VM * M, p2_y_act[:, 1:2], p2_y_prd[:, 1:2], 'Reactive Process-2 Event Processing Case', '2')
    fdh_graph.plt_show2_2(Z_VM * M, normal_p2_ez_all_run_q2[:, 1:2], reactive_p2_ez_all_run_q2[:, 1:2], 'Reactive Process-2 Event Processing Case', '2', color1='bx-', color2='rx--')
